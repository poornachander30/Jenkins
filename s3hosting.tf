...
resource "null_resource" "nullmount" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/poornachander30/WebsiteonS3.git /var/www/html"
    ]
  }
}
...
resource "aws_s3_bucket" "purna2610141822262834end2" {
  bucket = "purna2610141822262834end2"
...
#   region = "ap-southeast-1"
  ...
  tags = {
    Name = "purna2610141822262834end2"
    Environment = "Prod"
  }

 provisioner "local-exec" {
    command = "git clone https://github.com/poornachander30/WebsiteonS3.git web-server-image"
 }
...
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.purna2610141822262834end2]
 bucket = "purna2610141822262834end2"
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.purna2610141822262834end2.bucket_regional_domain_name
